...
function getLocalizedCopy(language: Language) {
  if (isRuFamilyLanguage(language)) {
    return {
      liveResultKicker: "Живой результат промпта",
      liveResultSubtitle: "Сначала вы видите итог AI-работы. Ниже — полный промпт и доступ к его логике.",
      controlKicker: "Промпт в действии",
      ...
      lockedTitle: "Полный промпт закрыт",
      lockedBody: "Free-пользователь видит результат и демо. PRO открывает полный blueprint, копирование и кастомизацию.",
      unlockCta: "Разблокировать промпт (PRO)",
      telegramCta: "Продолжить в Telegram-боте",
      unlockedTitle: "Полный промпт доступен",
      unlockedBody: "Вы можете копировать и адаптировать промпт под свои данные прямо сейчас.",
      runNow: "Запустить промпт",
      runPending: "Запуск...",
      ...
      demoIpCapReached: "Достигнут дневной лимит гостевых запусков для этого промпта.",
      demoFingerprintCapReached: "Достигнут дневной лимит по отпечатку устройства для этого промпта.",
  ...
  return {
    liveResultKicker: "Live prompt output",
    liveResultSubtitle: "You see the AI result first. The full prompt logic is available below.",
    controlKicker: "Prompt in action",
    ...
    unlockCta: "Unlock prompt (PRO)",
    telegramCta: "Continue in Telegram bot",
    unlockedTitle: "Full prompt is available",
    unlockedBody: "You can copy and customize this prompt right now.",
    runNow: "Run prompt",
    runPending: "Running...",
    demoRunsLeft: "Demo runs left: {count}",
    demoUnlimited: "PRO: unlimited runs",
    demoCapReached: "Demo run cap reached. Upgrade to PRO.",
    demoIpCapReached: "Guest daily run limit reached for this prompt.",
    demoFingerprintCapReached: "Guest fingerprint daily run limit reached for this prompt.",
