...
import type { WorkspaceMapCardProps } from "@/components/dashboard/WorkspaceMapCard";
...
export function buildWorkspaceCards({
...
}: BuildWorkspaceCardsArgs): WorkspaceMapCardProps[] {
  const activeCoursesCount = learningMy?.active_courses.length ?? 0;
  ...
  return [
    ...
    },
    {
      ...
      statusLabel: needsOnboarding
        ? t("dashboard.navStatusOnboarding")
        : activeCoursesCount > 0
          ? t("dashboard.navStatusActiveCourses", { count: activeCoursesCount })
          : completedCoursesCount > 0
            ? t("dashboard.navStatusCompletedCourses", { count: completedCoursesCount })
            : t("dashboard.navStatusNew"),
      statusTone: needsOnboarding ? "warning" : activeCoursesCount > 0 ? "success" : "neutral",
    ...
    },
    {
      ...
      statusLabel:
        typeof balance === "number" && balance > 0
          ...
          : t("dashboard.navStatusNew"),
      statusTone: typeof balance === "number" && balance > 0 ? "info" : "neutral",
