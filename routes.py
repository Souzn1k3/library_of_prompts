...
async def about(message: Message):
    await message.answer(
        ...
        "🔹 находить полезные промпты и решения для учёбы, работы и творчества и т. п.;\n"
        "🔹 изучать основы prompt engineering — искусства составления эффективных запросов к ИИ;\n"
        "🔹 отправлять свои промпты на AI-анализ и получать рекомендации по улучшению;\n"
        "🔹 практиковаться через миссии, мини-игры и задания;\n"
        "🔹 пользоваться разными нейросетями в одном удобном интерфейсе.\n\n"
        "Наша цель — сделать работу с искусственным интеллектом понятной, доступной и реально полезной для каждого: от новичка до человека, который хочет применять ИИ профессионально."
    )
