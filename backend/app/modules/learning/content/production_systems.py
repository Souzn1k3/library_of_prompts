                    ...
                    "steps": [
                        {
                            ...
                            "estimated_minutes": 6,
                            "content": [
                                tr("Production prompting starts with a system spec: inputs, stage contracts, outputs, and explicit success rules.", "Продакшен-промптинг начинается не с красивого текста промпта, а с системной спецификации. Сначала нужно описать входы, этапы, выходы и правила успеха, и только потом думать о формулировках для модели.", "Продакшен-промптинг system specтан башлана: керүләр, этап контрактлары, нәтиҗәләр һәм ачык уңыш кагыйдәләре."),
                                tr("A spec turns prompting from artisanal writing into an operating process that can be reviewed by others.", "Системная спецификация переводит промптинг из ремесленного письма в операционный процесс. Когда она оформлена нормально, другой человек может понять систему, проверить ее и доработать без постоянных уточнений у автора.", "Спецификация промптингны кул эшеннән команда ревьюлый ала торган операцион процесска күчерә."),
                                tr("Interfaces should define what each stage receives and what it must return; otherwise stage boundaries collapse.", "Интерфейсы должны четко задавать, что получает каждый этап и что он обязан вернуть. Например, этап анализа получает заметки и ссылки, а возвращает таблицу claims с evidence. Если это не зафиксировано, этапы начинают смешиваться, а скрытые допущения ползут дальше по цепочке.", "Интерфейслар һәр этапның нәрсә алуын һәм нәрсә кайтарырга тиешлеген билгеләргә тиеш, югыйсә этап чиге җимерелә."),
                                tr("When interfaces are fuzzy, prompt text absorbs hidden complexity and becomes brittle under variation.", "Когда интерфейсы размыты, вся скрытая сложность падает прямо в текст промпта. В итоге он становится длинным, нервным и хрупким: чуть меняется задача, источник данных или пользователь, и система перестает работать предсказуемо.", "Интерфейс томан булганда, промпт тексты яшерен катлаулылыкны йота һәм үзгәрешләргә какшак була."),
                                tr("A review-ready spec must be testable: another teammate should detect pass/fail without asking the author.", "Хорошая спецификация должна быть тестируемой. Если другой участник команды не может без подсказок понять, что здесь считается pass, что fail и где проверять качество, значит спецификация еще не готова к реальной работе.", "Ревьюга яраклы спецификация тестлана торган булырга тиеш: башка команда әгъзасы pass/failны автордан сорамыйча ачыкларга тиеш."),
                            ],
                    ...
                    "steps": [
                        {
                            ...
                            "estimated_minutes": 6,
                            "content": [
                                tr("Evaluation harness is the quality memory of a prompt system: it tells whether changes help or harm.", "Evaluation harness — это память качества prompt-системы. Он нужен, чтобы после каждой правки не спорить на уровне ощущений, а видеть, действительно система стала лучше или просто случайно выдала удачный ответ один раз.", "Evaluation harness prompt-системаның сыйфат хәтере: үзгәрешнең файда китерүенме яки зыян салуынмы күрсәтә."),
                                tr("If pass/fail is undefined, every tweak looks promising and regressions stay invisible.", "Если pass и fail не определены заранее, почти любая правка кажется улучшением. Именно поэтому регрессии так долго живут в продакшене: команда радуется новой версии, а потом замечает, что на соседних кейсах она стала хуже.", "Pass/fail билгеләнмәсә, һәр үзгәреш «яхшырту» булып күренә, ә регрессияләр күренми кала."),
                                tr("A minimal but strong harness includes sample cases, failure labels, threshold rules, and report structure.", "Даже минимальный, но сильный harness должен включать набор тестовых кейсов, ярлыки сбоев, правила порога и понятную структуру отчета. Тестовые кейсы показывают, на каких примерах вы проверяете систему, ярлыки описывают типы сбоев, порог задает границу качества, а отчет делает результаты сравнимыми между версиями.", "Минималь ләкин көчле harness sample cases, хата ярлыклары, threshold кагыйдәләре һәм хисап структурасын кертә."),
                                tr("Failure labels must be actionable: they should point to concrete repair decisions, not vague criticism.", "Failure labels обязаны быть прикладными. Ярлык вроде «плохой ответ» бесполезен, а ярлык «off-policy», «неполный», «придумал факт» или «неверный формат» уже подсказывает, какую именно часть системы нужно чинить.", "Хата ярлыклары гамәлгә илтергә тиеш: томан тәнкыйтькә түгел, төгәл төзәтүгә күрсәтергә."),
                                tr("Measure trend over time, not one run: production quality is stability across iterations.", "Смотреть нужно не на один красивый запуск, а на тренд во времени. Продакшен-качество — это стабильность на серии кейсов и в нескольких итерациях. Если версия блестяще проходит один пример, но сыпется на соседних, она еще не готова к реальной нагрузке.", "Бер запускка түгел, вакыт буенча трендка карагыз: production-сыйфат итерацияләрдә тотрыклылык ул."),
                            ],
                            "submission": {"type": "none"},
                        },
                        {
                    ...
                    "steps": [
                        {
                            ...
                            "estimated_minutes": 6,
                            "content": [
                                tr("Context control is selective exposure: decide what the system must see, may see, and must ignore.", "Контроль контекста — это селективная подача. Мы заранее решаем, что система обязана видеть, что может получить как дополнительный материал, а что должна игнорировать полностью. Иначе модель начинает тратить внимание на шум и случайные детали.", "Контекст контроле - сайлап бирү: система нәрсәне мәҗбүри күрә, нәрсәне өстәмә күрә һәм нәрсәне читкә куя."),
                                tr("More context is not always better; excess noise degrades reasoning and increases contradiction risk.", "Больше контекста не всегда лучше. Anthropic отдельно пишет, что при длинном контексте важно структурировать документы, выносить длинные материалы вверх и четко отделять запрос от данных. Практический смысл простой: не сыпьте в промпт все подряд, иначе вместе с полезной информацией вы приносите противоречия и мусор.", "Күбрәк контекст һәрвакыт яхшы түгел: артык шау фикер йөртүне боза һәм каршылык рисгын арттыра."),
                                tr("Guardrails are operational rules: trigger conditions plus fallback actions and escalation ownership.", "Guardrails — это не расплывчатое пожелание «будь осторожен», а конкретные операционные правила. У guardrail всегда есть trigger-условие, действие по умолчанию и понятный владелец эскалации. Например: если нет подтвержденной policy, не отвечай уверенно, а подними флаг и отправь кейс человеку.", "Guardrails - операцион кагыйдәләр: trigger шартлары, fallback гамәлләре һәм эскалация ownershipы."),
                                tr("Stage handoff contracts prevent hidden assumptions when context moves between steps.", "Очень важны и handoff-контракты между этапами. Когда контекст передается от шага к шагу, нужно заранее договориться, что именно переносим дальше: только подтвержденные факты, только сжатое summary, таблицу решений или весь документ. Без этого следующий этап начинает додумывать пропущенные куски.", "Этаплар арасындагы handoff контрактлары контекст күчкәндә яшерен фаразларны кисәтә."),
                                tr("Reliable systems fail safely: they stop guessing and escalate when confidence drops below threshold.", "Надежные системы ошибаются безопасно. Это значит, что при нехватке данных, конфликте источников или падении confidence ниже порога они прекращают угадывать и эскалируют ситуацию. Для продакшена это не слабость, а признак зрелости системы.", "Ышанычлы системалар куркынычсыз рәвештә туктый: ышаныч порогтан төшсә, фаразны туктата һәм эскалацияли."),
                            ],
                            "submission": {"type": "none"},
                        },
                        {
                    ...
                    "steps": [
                        {
                            ...
                            "estimated_minutes": 6,
                            "content": [
                                tr("Capstone means production intent: deliver a package another teammate can run, review, and maintain.", "Капстоун здесь означает production-намерение. Вы сдаете не просто промпт и не просто красивую схему, а пакет, который другой участник команды сможет запустить, проверить и сопровождать без постоянного созвона с вами.", "Капстоун production ниятен аңлата: башка команда әгъзасы эшләтә, ревьюлый һәм алып бара ала торган пакет тапшырыгыз."),
                                tr("Your package should read like an operating note: purpose, interfaces, checks, fallback, owner, cadence.", "Хороший пакет читается как операционная памятка: цель системы, интерфейсы, входы, этапы, проверки, fallback, владелец процесса, ритм запуска и метрика успеха. Человек должен открыть документ и быстро понять, что система делает, где ее границы и что происходит, когда все идет не по плану.", "Пакет operating note кебек укылырга тиеш: максат, интерфейслар, тикшерү, fallback, owner, cadence."),
                                tr("Do not optimize for perfect wording; optimize for reliable operation under normal and failure conditions.", "Оптимизировать нужно не красоту формулировки, а надежную работу в обычных и аварийных условиях. Процесс production-уровня хорош не тогда, когда он звучит умно, а тогда, когда одинаково понятно ведет себя на нормальном кейсе, на пограничном кейсе и в ситуации, где данных явно не хватает.", "Максатыгыз матур формулировка түгел, ә гадәти һәм авария шартларында ышанычлы эш булсын."),
                                tr("A complete submission includes measurable success criteria and first-response actions for quality drops.", "Полная работа обязательно включает измеримые критерии успеха и первые действия на случай просадки качества. То есть пользователь должен увидеть не только идеальный сценарий, но и очень конкретный план: по какому сигналу мы понимаем, что качество упало, и что делаем в первые минуты после этого.", "Тулы эштә үлчәнә торган уңыш критерийлары һәм сыйфат төшкәндә беренче реакция гамәлләре булырга тиеш."),
                            ],
                            "submission": {"type": "none"},
                        },
                        {
