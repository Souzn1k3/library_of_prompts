...
class User(Base):
    ...
    scenario_game_events: Mapped[list["ScenarioGameTokenEvent"]] = relationship(
        back_populates="user",
        cascade="all, delete-orphan",
    )
    scenario_game_claims: Mapped[list["ScenarioGameTokenClaim"]] = relationship(
        back_populates="user",
        cascade="all, delete-orphan",
    )
